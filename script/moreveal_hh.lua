...
local script_version = 23 --[[ Используется для автообновления, во избежание проблем 
с получением новых обновлений, рекомендуется не изменять. В случае их появления измените значение на "1" ]]
local text_version = '1.2' -- версия для вывода в окне настроек, не изменять
...
function main()
    ...
    if not doesDirectoryExist(getWorkingDirectory()..'/config/Hitman Helper') then
        createDirectory(getWorkingDirectory()..'/config/Hitman Helper')
    end
    ...
    if not doesFileExist(getWorkingDirectory()..'/lib/requests.lua') then
        downloadUrlToFile('https://www.dropbox.com/s/ytymzr9p8bw6vui/requests.lua?dl=1', getWorkingDirectory()..'/lib/requests.lua', function(id, status) 
    ...
    local f
    if not doesFileExist(anonymizer_path) then
        f = io.open(anonymizer_path, 'w+')
    else
        f = io.open(anonymizer_path, 'r+')
    end
    for line in f:lines() do
    ...
    if mainIni.config.anonymizer then
        ...
        for k, v in pairs(anonymizer_names) do
            local name = v:match('(.+) =')
            local mask = v:match('= (.+)')
            if sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))) == name or sampIsPlayerConnected(sampGetPlayerIdByNickname(mask)) then
    ...
    if script_version < tonumber(new_version) then
    ...
    last_news = u8:decode(requests.get('https://raw.githubusercontent.com/moreveal/moreveal_hh/main/script/last_news.txt').text)
    ...
    sampRegisterChatCommand('cfd', function(arg)
        if cfd == nil then
            if arg:find('%D') or #arg == 0 then
                sampAddChatMessage('[ Мысли ]: Правильное использование поиска: [/cfd ID]', 0xCCCCCC)
            else
    ...
    sampRegisterChatCommand('otstrel_list', function()
        if mainIni.config.otstrel then
            ...
            local dialog_text
            if table.maxn(otstrel_online) > 0 then
                ...
                sampAddChatMessage('[ Отстрел ]: Список пуст..', 0xCCCCCC)
            end
        else
    ...
    while true do
        ...
        lua_thread.create(function ()
            if update then
                downloadUrlToFile('https://github.com/moreveal/moreveal_hh/raw/main/script/moreveal_hh.lua', thisScript().path, function(id, status)
                    if status == dlstatus.STATUS_ENDDOWNLOADDATA then
        ...
        if c_cfd_hp then
            lua_thread.create(function ()
                wait(300)
                if sampGetPlayerHealth(cfd) <= 0 then
...
function sampev.onPlayerQuit(playerid, reason)
    if mainIni.config.otstrel then
        for k, v in pairs(otstrel_online) do
            if otstrel_online[k]['id'] == playerid then
                sampAddChatMessage('[ Отстрел ]: '..nick..' ['..playerid..'] покинул сервер.', 0xCCCCCC)
            end
        end
    end
end
...
function layoutMacrossString(m_key)
    ...
    for k, v in pairs(macrosses_list[m_key]) do
        ...
        if macrosses_list[m_key][k + 1] ~= nil then
            if string == nil then string = key..' + ' else string = string..key..' + ' end
        else
            if string == nil then string = key else string = string..key end
        end
    end
    if string == nil then return 'Не назначено' end
    return string
end

function sampev.onSendGiveDamage(playerid, damage, weapon, bodypart)
    if playerid == cfd then
        c_cfd_hp = true
    end
end
...
if cmd:find('^/id ') then
        if mainIni.config.customid then
            ...
            for k, v in pairs(nick_colours) do
                if target_lower:find(string.nlower(v)) then
                    for i = 0, sampGetMaxPlayerId() do
                        if sampIsPlayerConnected(i) and sampGetPlayerColor(i) == k then
            ...
            for i = 0, sampGetMaxPlayerId() do
                if sampIsPlayerConnected(i) then
                    if string.nlower(sampGetPlayerNickname(i)):find(target_lower) then
                        table.insert(players, {name = sampGetPlayerNickname(i), id = i, colour = sampGetPlayerColor(i), level = sampGetPlayerScore(i), ping = sampGetPlayerPing(i)})
                    end
                end
            end
            if table.maxn(players) ~= 0 then
                ...
                for i = 1, table.maxn(players) do
                    if nick_colours[players[i]['colour']] == nil then colour = 'Неизвестно' else colour = nick_colours[players[i]['colour']] end
                ...
                if not target:find('%D') then
                    if sampIsPlayerConnected(tonumber(target_lower)) then
                        target = tonumber(target_lower)
                        if nick_colours[sampGetPlayerColor(target_lower)] == nil then colour = 'Неизвестно' else colour = nick_colours[sampGetPlayerColor(target)] end
...
function sampev.onServerMessage(color, text)
    ...
    if mainIni.config.customctstr then
        if text:find('%*%* %{......%}Агент №(%d+) %{......%}принял контракт на: %{......%}.+%[(%d+)%] %{......%}Цена: (%d+)$ %{......%}%*%*') then
            ...
            return {color, '{ffff00}Агент №'..number..'{0088ff} принял {ffff00}контракт на: {'..colour..'}'..sampGetPlayerNickname(id):gsub('_', ' ')..'{ffff00} | Цена: {008000}'..price..'$'}
        end
        ...
        if text:find('%{......%}« %{......%}Агент № (%d+) выполнил контракт на .+, и получил %{......%}(%d+)$ %{......%}»') then
            ...
            local colour = string.format('%06X', bit.band(sampGetPlayerColor(id),  0xFFFFFF))
            return {color, '{ffff00}Агент №'..number..' {008000}выполнил{ffff00} контракт на {'..colour..'}'..nick:gsub('_', ' ')..'{ffff00} ['..id..'] получив {008000}'..price..'$'}
        end
        ...
        if text:find('%{......%}%*%* %{......%}Агент №(%d+) %{......%}отказывается выполнять контракт на: %{......%}.+%[(%d+)%] %{......%}%*%*') then
            ...
            local colour = string.format('%06X', bit.band(sampGetPlayerColor(id),  0xFFFFFF))
            return {color, '{ffff00}Агент №'..number..' {ff0000}отказывается{ffff00} выполнять контракт на {'..colour..'}'..sampGetPlayerNickname(id):gsub('_', ' ')..'{ffff00} ['..id..']'} 
        end
    end
    ...
    if not mainIni.chat.ads then
        if text:find('%* %[.*Реклама%]:%{......%}.+, %{......%}Контакт') or text:find('%* Обработал:{......} .+ %*') then
            print(text)
            return false
        end
    end
    ...
    if text:find('сбил с ног .+, ударом по голове.') then
        lastknocked = text:match('сбил с ног (.+), ударом по голове.')
    end
end
...
function macrossesFunc()
    if mainIni.config.macrosses then
        ...
        elseif isKeysDown(macrosses_list.boot, true) and not sampIsChatInputActive() then
            goKeyPressed(78)
            lua_thread.create(function ()
                while not sampTextdrawIsExists(2204) do
                    wait(0)
                end

                sampSendClickTextdraw(2204)

                while not sampIsDialogActive(899) do
                    wait(0)
                end

                sampSendDialogResponse(899, 1, 1, -1)

                while not sampIsDialogActive(547) do
                    wait(0)
                end

                if lastknocked ~= nil then
                    sampSendDialogResponse(547, 1, 1, lastknocked)
                end
            end)
            wait(300)
        ...
        elseif isKeysDown(macrosses_list.otstrel, true) and not sampIsChatInputActive() then
            sampProcessChatInput('/otstrel_list')
            wait(300)
        ...
        if not input:find('.+ = .+') then
            ...
            if not retry then
                ...
                if mainIni.config.anonymizer then
                    ...
                    if sampGetPlayerNickname(localid) == name or sampIsPlayerConnected(sampGetPlayerIdByNickname(name)) then
                        changeName(name, mask)
                    end
                end
            else
        ...
        if button == 1 then
            ...
            if listitem == 15 then
                ...
                for k, v in pairs(weapons_list) do
                    if weapon_line == nil then
                        weapon_line = 'Текущее название оружия\tНовое значение\n'..v..'\t>>\n'
                    else
                        weapon_line = weapon_line..v..'\t>>\n'
                    end
                end
            ...
            if listitem == 18 then
                local keys_str = layoutMacrossString('screen')
                sampAddChatMessage('[ Hitman Helper ]: После выполненного контракта скрипт автоматически нажимает сочетание клавиш [ '..keys_str..' ]', 0xCCCCCC)
                ...
                sampAddChatMessage('[ Hitman Helper ]: Нажмите F4, чтобы скрипт нажал сочетание клавиш [ '..keys_str..' ], либо F5, чтобы выйти из этого режима', 0xCCCCCC)
                test_as = true
                openMenu = false
            end
...
function scriptBody()
    ...
    displayHud((not mainIni.config.hud and true or false))
    if mainIni.config.hud then
        ...
        local weaponline
        if weapon ~= nil and weapon <= 19 then
            weaponline = string.upper(weapons_list[weapon + 1])..(weapon > 15 and weapon ~= 46 and ' ('..getAmmoInClip() ..'/'.. getAmmoInCharWeapon(PLAYER_PED, weapon) - getAmmoInClip()..')' or '')
        else
            weaponline = string.upper(weapons_list[weapon])..(weapon > 15 and weapon ~= 46 and ' ('..getAmmoInClip() ..'/'.. getAmmoInCharWeapon(PLAYER_PED, weapon) - getAmmoInClip()..')' or '')
        end

        renderDrawBox(mainIni.hud.xpos, mainIni.hud.ypos, 357, 30, 0xFF000000)
        renderDrawBox(mainIni.hud.xpos - 0.568, mainIni.hud.ypos + 27, math.ceil(3.57 * getCharArmour(PLAYER_PED)), 5, 4291285986.0)
        renderDrawBox(mainIni.hud.xpos, getCharArmour(PLAYER_PED) ~= 0 and mainIni.hud.ypos + 33 or mainIni.hud.ypos + 27, 357, 5, 4281150765.0)
    
        renderDrawBox(mainIni.hud.xpos - 0.568, getCharArmour(PLAYER_PED) ~= 0 and mainIni.hud.ypos + 33 or mainIni.hud.ypos + 27, math.ceil(3.57 * health), 5, 4291722364.0)
        renderDrawLine(mainIni.hud.xpos - 0.44, mainIni.hud.ypos - 1, mainIni.hud.xpos + 357, mainIni.hud.ypos - 1, 1.5, 0xFFFFFFFF)
        ...
        if getCurrentCharWeapon(PLAYER_PED) ~= 0 and (not isCharInAnyCar(PLAYER_PED) or PLAYER_PED ~= getDriverOfCar(storeCarCharIsInNoSave(PLAYER_PED))) then
            renderFontDrawText(font_hud, setpoint(money) .. "$", mainIni.hud.xpos + 2, mainIni.hud.ypos + 2, 4294967295.0)
        ...
        elseif isCharInAnyCar(PLAYER_PED) and PLAYER_PED == getDriverOfCar(storeCarCharIsInNoSave(PLAYER_PED)) then
            renderFontDrawText(font_hud, setpoint(money) .. "$", mainIni.hud.xpos + 180 - renderGetFontDrawTextLength(font_hud, setpoint(getPlayerMoney(localPlayer))) / 2, mainIni.hud.ypos + 3, 4294967295.0)
            --renderFontDrawText(font_hud, setpoint(money) .. "$", mainIni.hud.xpos + 2, mainIni.hud.ypos + 2, 4294967295.0)
        else
            renderFontDrawText(font_hud, setpoint(money) .. "$", mainIni.hud.xpos + 180 - renderGetFontDrawTextLength(font_hud, setpoint(getPlayerMoney(localPlayer))) / 2, mainIni.hud.ypos + 3, 4294967295.0)
        end
    ...
    if cfd ~= nil then
        ...
        if not isPauseMenuActive() and sampIsPlayerConnected(tonumber(cfd)) then
            renderFontDrawText(font, '{ff0000}ПОИСК: {ffffff}'..sampGetPlayerNickname(cfd):gsub('_', ' ')..' [ '..cfd..' ]', mainIni.hud.xpos - 1, mainIni.hud.ypos - 33, 0xFFFFFFFF, 1)
    ...
    if getInvisiblity(id) then
        if cfd ~= nil then
            renderFontDrawText(font, '{0088ff}НЕВИДИМОСТЬ', mainIni.hud.xpos - 1, mainIni.hud.ypos - 60, 0xFFFFFFFF)
        else
            renderFontDrawText(font, '{0088ff}НЕВИДИМОСТЬ', mainIni.hud.xpos - 1, mainIni.hud.ypos - 33, 0xFFFFFFFF)
        end
    end

    if cfd ~= nil then
        renderFontDrawText(font, 'NAMETAG ['..(not nametag and '{008000} ON ' or '{ff0000} OFF ')..'{ffffff}]', mainIni.hud.xpos + 217.88, mainIni.hud.ypos - 33, 0xFFFFFFFF, 1)
    else
        if getInvisiblity(id) then
            renderFontDrawText(font, 'NAMETAG ['..(not nametag and '{008000} ON ' or '{ff0000} OFF ')..'{ffffff}]', mainIni.hud.xpos + 114.2, mainIni.hud.ypos - 33, 0xFFFFFFFF, 1)
        else
            renderFontDrawText(font, 'NAMETAG ['..(not nametag and '{008000} ON ' or '{ff0000} OFF ')..'{ffffff}]', mainIni.hud.xpos - 1, mainIni.hud.ypos - 33, 0xFFFFFFFF, 1)
        end
    end
end
...
function onWindowMessage(msg, wparam, lparam)
    if msg == 0x100 or msg == 0x104 then
    	if bit.band(lparam, 0x40000000) == 0 then
			if (setting_bind ~= nil) and (wparam ~= 18 and wparam ~= 16 and wparam ~= 17) then
...
function onScriptTerminate(script, quit)
    if script == thisScript() then
        ...
        for k, v in pairs(anonymizer_names) do
            if anonymizer_names[k + 1] ~= nil then
                f:write(u8(v)..'\n')
            else
                f:write(u8(v))
            end
        ...
        for k, v in pairs(macrosses_list) do
            for kt, vt in pairs(macrosses_list[k]) do
                if macrosses_list[k][kt + 1] ~= nil then 
					if m_string == nil then 
						m_string = vt..' + ' 
					else
						m_string = m_string..vt..' + '
					end
                else
					if m_string == nil then
						m_string = vt
