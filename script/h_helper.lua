...
local script_version = 32 --[[ Используется для автообновления, во избежание проблем 
            ...
            if mainIni.config.autoupdate then
                downloadUrlToFile('https://www.dropbox.com/s/ayjqaiu7n9nun7t/h_helper.lua?dl=1', thisScript().path, function(id, status)
                    if status == dlstatus.STATUS_ENDDOWNLOADDATA then
                        ...
                        print('Ссылка на актуальную версию: dropbox.com/s/ayjqaiu7n9nun7t/h_helper.lua')
                        thisScript():reload()
                    end 
                end)
                update = false
...
function screenct()
    ...
    sampAddChatMessage('Contract framed', 0x850000)
end
